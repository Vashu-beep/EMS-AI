...
export default function EmployeesPage() {
  ...
  const [debouncedSearch] = useDebounce(search, 400);  // ✅ reduces API calls
  ...
  const fetchEmployees = async () => {
    ...
    try {
      ...
      // ✅ filter out any undefined departments
      const uniqueDepts = [
  ...
  const deactivate = async (id: string) => {
    if (!confirm('Deactivate employee? This can be undone.')) return;
    try {
            ...
            {employees.map((emp) => (
              <tr key={emp._id} className="border-t">
                ...
                <td className="p-2 space-x-2">
                  <Link href={`/employees/${emp._id}`} className="text-blue-600 hover:underline">
                    View
                  </Link>
                  <Link href={`/employees/${emp._id}/edit`} className="text-green-600 hover:underline">
                    Edit
                  </Link>
                  <button
                    onClick={() => deactivate(emp._id)}
